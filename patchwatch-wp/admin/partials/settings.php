<?php
/**
 * Admin partial: Settings page.
 *
 * @package PatchWatch
 * @var array $settings Current settings from PWW_Utils::get_settings().
 */

if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
?>
<div class="wrap aipatch-wrap">
    <h1 class="aipatch-page-title">
        <span class="dashicons dashicons-admin-generic"></span>
        <?php esc_html_e( 'AI PatchWatch Settings', 'patchwatch-wp' ); ?>
    </h1>

    <form method="post" action="options.php">
        <?php settings_fields( 'aipatch_settings_group' ); ?>

        <table class="form-table aipatch-settings-table" role="presentation">
            <!-- Scan Frequency -->
            <tr>
                <th scope="row">
                    <label for="aipatch_scan_frequency"><?php esc_html_e( 'Scan Frequency', 'patchwatch-wp' ); ?></label>
                </th>
                <td>
                    <select name="aipatch_settings[scan_frequency]" id="aipatch_scan_frequency">
                        <option value="twicedaily" <?php selected( $settings['scan_frequency'], 'twicedaily' ); ?>>
                            <?php esc_html_e( 'Twice Daily', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="daily" <?php selected( $settings['scan_frequency'], 'daily' ); ?>>
                            <?php esc_html_e( 'Daily', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="weekly" <?php selected( $settings['scan_frequency'], 'weekly' ); ?>>
                            <?php esc_html_e( 'Weekly', 'patchwatch-wp' ); ?>
                        </option>
                    </select>
                    <p class="description">
                        <?php esc_html_e( 'How often the automatic security scan should run.', 'patchwatch-wp' ); ?>
                    </p>
                </td>
            </tr>

            <!-- Log Retention -->
            <tr>
                <th scope="row">
                    <label for="aipatch_log_retention"><?php esc_html_e( 'Log Retention', 'patchwatch-wp' ); ?></label>
                </th>
                <td>
                    <select name="aipatch_settings[log_retention_days]" id="aipatch_log_retention">
                        <option value="7" <?php selected( $settings['log_retention_days'], 7 ); ?>>
                            <?php esc_html_e( '7 days', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="14" <?php selected( $settings['log_retention_days'], 14 ); ?>>
                            <?php esc_html_e( '14 days', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="30" <?php selected( $settings['log_retention_days'], 30 ); ?>>
                            <?php esc_html_e( '30 days', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="60" <?php selected( $settings['log_retention_days'], 60 ); ?>>
                            <?php esc_html_e( '60 days', 'patchwatch-wp' ); ?>
                        </option>
                        <option value="90" <?php selected( $settings['log_retention_days'], 90 ); ?>>
                            <?php esc_html_e( '90 days', 'patchwatch-wp' ); ?>
                        </option>
                    </select>
                    <p class="description">
                        <?php esc_html_e( 'How long to keep log entries before automatic cleanup.', 'patchwatch-wp' ); ?>
                    </p>
                </td>
            </tr>

            <!-- REST Compatibility Mode -->
            <tr>
                <th scope="row">
                    <?php esc_html_e( 'REST API Compatibility Mode', 'patchwatch-wp' ); ?>
                </th>
                <td>
                    <label>
                        <input type="checkbox" name="aipatch_settings[rest_compat_mode]" value="1" <?php checked( $settings['rest_compat_mode'] ); ?> />
                        <?php esc_html_e( 'Enable compatibility mode', 'patchwatch-wp' ); ?>
                    </label>
                    <p class="description">
                        <?php esc_html_e( 'When enabled, REST API restrictions are relaxed to ensure maximum compatibility with third-party plugins and headless setups. This bypasses the REST API hardening filter.', 'patchwatch-wp' ); ?>
                    </p>
                </td>
            </tr>

            <!-- Modules -->
            <tr>
                <th scope="row">
                    <?php esc_html_e( 'Active Modules', 'patchwatch-wp' ); ?>
                </th>
                <td>
                    <fieldset>
                        <label>
                            <input type="checkbox" name="aipatch_settings[modules_enabled][scanner]" value="1" <?php checked( ! empty( $settings['modules_enabled']['scanner'] ) ); ?> />
                            <?php esc_html_e( 'Security Scanner', 'patchwatch-wp' ); ?>
                        </label><br>

                        <label>
                            <input type="checkbox" name="aipatch_settings[modules_enabled][hardening]" value="1" <?php checked( ! empty( $settings['modules_enabled']['hardening'] ) ); ?> />
                            <?php esc_html_e( 'Hardening Module', 'patchwatch-wp' ); ?>
                        </label><br>

                        <label>
                            <input type="checkbox" name="aipatch_settings[modules_enabled][vulnerabilities]" value="1" <?php checked( ! empty( $settings['modules_enabled']['vulnerabilities'] ) ); ?> />
                            <?php esc_html_e( 'Vulnerability Intelligence', 'patchwatch-wp' ); ?>
                        </label><br>

                        <label>
                            <input type="checkbox" name="aipatch_settings[modules_enabled][login_protection]" value="1" <?php checked( ! empty( $settings['modules_enabled']['login_protection'] ) ); ?> />
                            <?php esc_html_e( 'Login Protection', 'patchwatch-wp' ); ?>
                        </label>
                    </fieldset>
                    <p class="description">
                        <?php esc_html_e( 'Enable or disable individual plugin modules.', 'patchwatch-wp' ); ?>
                    </p>
                </td>
            </tr>
        </table>

        <?php submit_button( __( 'Save Settings', 'patchwatch-wp' ) ); ?>
    </form>

    <div class="aipatch-section aipatch-muted-box">
        <h3><?php esc_html_e( 'Plugin Information', 'patchwatch-wp' ); ?></h3>
        <table class="aipatch-info-table">
            <tr>
                <td><strong><?php esc_html_e( 'Version', 'patchwatch-wp' ); ?></strong></td>
                <td><?php echo esc_html( AIPATCH_VERSION ); ?></td>
            </tr>
            <tr>
                <td><strong><?php esc_html_e( 'Database Version', 'patchwatch-wp' ); ?></strong></td>
                <td><?php echo esc_html( get_option( 'aipatch_db_version', 'N/A' ) ); ?></td>
            </tr>
            <tr>
                <td><strong><?php esc_html_e( 'PHP Version', 'patchwatch-wp' ); ?></strong></td>
                <td><?php echo esc_html( PHP_VERSION ); ?></td>
            </tr>
            <tr>
                <td><strong><?php esc_html_e( 'WordPress Version', 'patchwatch-wp' ); ?></strong></td>
                <td><?php echo esc_html( get_bloginfo( 'version' ) ); ?></td>
            </tr>
        </table>
    </div>
</div>
