...
class PWW_Hardening {
        ...
        if ( $result ) {
            ...
            $this->logger->info(
                ...
                sprintf(
                    ...
                    __( 'Hardening option "%1$s" %2$s.', 'aipatch-security-scanner' ),
                    $key,
                    $action
                )
            );
    ...
    public function get_status() {
        ...
        return array(
            array(
                'key'           => 'disable_xmlrpc',
                'title'         => __( 'Disable XML-RPC', 'aipatch-security-scanner' ),
                'description'   => __( 'Blocks external XML-RPC requests. Disable if you use the WordPress mobile app, Jetpack, or remote publishing tools.', 'aipatch-security-scanner' ),
                'enabled'       => ! empty( $options['disable_xmlrpc'] ),
                'warning'       => __( 'May break Jetpack and WordPress mobile app connectivity.', 'aipatch-security-scanner' ),
                'severity'      => 'medium',
            ),
            array(
                'key'           => 'hide_wp_version',
                'title'         => __( 'Hide WordPress Version', 'aipatch-security-scanner' ),
                'description'   => __( 'Removes the WordPress version number from the page source, RSS feeds, and scripts.', 'aipatch-security-scanner' ),
            ...
            array(
                ...
                'title'         => __( 'Restrict REST API', 'aipatch-security-scanner' ),
                'description'   => __( 'Limits sensitive REST API endpoints (like user enumeration) to authenticated users only. Public endpoints for themes and plugins remain accessible.', 'aipatch-security-scanner' ),
            ...
            array(
                ...
                'title'         => __( 'Login Brute-Force Protection', 'aipatch-security-scanner' ),
                'description'   => sprintf(
                    /* translators: 1: Max attempts, 2: Lockout duration in minutes. */
                    __( 'Limits login attempts to %1$d tries per IP, with a %2$d-minute lockout after exceeding the limit.', 'aipatch-security-scanner' ),
                    $options['login_max_attempts'],
                    $options['login_lockout_duration']
                ),
                'enabled'       => ! empty( $options['login_protection'] ),
                'warning'       => __( 'Legitimate users may be locked out temporarily if they forget their password.', 'aipatch-security-scanner' ),
                'severity'      => 'high',
                'settings'      => array(
        ...
        add_filter( 'rest_authentication_errors', function ( $result ) {
            ...
            foreach ( $blocked_patterns as $pattern ) {
                if ( strpos( $request_uri, $pattern ) !== false ) {
                    return new WP_Error(
                        'rest_forbidden',
                        __( 'Authentication is required to access this endpoint.', 'aipatch-security-scanner' ),
                        array( 'status' => 401 )
                    );
                }
            }
            ...
            if ( $lockout ) {
                return new WP_Error(
                    ...
                    sprintf(
                        ...
                        __( 'Too many failed login attempts. Please try again in %d minutes.', 'aipatch-security-scanner' ),
                        $lockout_minutes
                    )
                );
            }
            ...
            if ( $attempts >= $max_attempts ) {
                ...
                $this->logger->warning(
                    ...
                    sprintf(
                        ...
                        __( 'IP locked out after %d failed login attempts.', 'aipatch-security-scanner' ),
                        $max_attempts
                    ),
